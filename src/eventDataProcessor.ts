...
function objectifyTsv(tableData: string) {
  ...
  // console.log('objectValueStrings:', objectValueStrings);
...
function createObjectArray(objectValueStrings: string[], objectKeys: string[]) {
  ...
  console.log('objectValueStrings.length:', objectValueStrings.length);
  objectValueStrings.forEach((objectValueString) => {
    if (objectValueStrings.length > 7) console.log('objectValueString:', objectValueString);
...
function makeNewObject(objectValueString: string, objectKeys: string[]) {
  ...
  console.log('returnObject.Date:', returnObject.Date);
  const newDay = new Intl.DateTimeFormat('en-US', { weekday: 'long' }).format(new Date(returnObject.Date));
  returnObject.dayName = newDay;
  [returnObject.monthNumber, returnObject.dayNumber, returnObject.yearNumber] = returnObject.Date.split('/');
